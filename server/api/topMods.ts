...
export default (app: Express) => {
    app.get("/api/v1/mods/top/:page{/:sort}", async (req, res) => {
        ...
        const sortTypes = ["downloads", "favorites"]
        if(sort && !sortTypes.some(v => v === sort)) {
            res.status(400).json({ success: false, message: "Sort type not found (options are downloads and favorites)" })
            return
        }

        const result = await database.methods.GetSearch(parseInt(page), undefined, sort as "downloads" | "favorites")
